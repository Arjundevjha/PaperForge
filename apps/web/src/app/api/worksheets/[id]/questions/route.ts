...
export async function GET(
  ...
  try {
    ...
    return new Response(Buffer.from(questionPdf), {
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `attachment; filename="${worksheet.worksheetNumber}_${worksheet.chapter.replace(/[^a-zA-Z0-9]/g, '_')}_Questions.pdf"`,
      },
    });
  } catch (err: any) {
